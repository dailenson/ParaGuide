...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, nargs=1, default=["IAM"])
    ...
    for i in tqdm(range(len(df)), leave=False):
        ...
        if img_name not in set(df_ref_writer['img_name']):
            ...
            ### measure the distances between reference set and random forgeries
            ref_writer = np.random.choice(list(writer_set - {writer}))
    ...
    # calculate evaluation metrics on both genuine samples and random forgeries
