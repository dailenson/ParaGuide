...
class SupConGuidanceLoss(nn.Module):
    """Supervised Contrastive Learning: https://arxiv.org/pdf/2004.11362.pdf.
    It also supports the unsupervised contrastive loss in SimCLR"""
    def __init__(self, temperature=0.07, contrast_mode='one',
                 base_temperature=0.07):
        super(SupConGuidanceLoss, self).__init__()
        self.temperature = temperature
        self.contrast_mode = contrast_mode
        self.base_temperature = base_temperature
...
class Triplet_Model(nn.Module):
    def __init__(self, args):
        ...
        self.criterion_guidance = SupConGuidanceLoss(contrast_mode='one')
    ...
    def train_iter(self, batch):
        # prototype
        pro_emb = self.pro_fea(self.encoder(batch['prototype_img'].to(device), pool=True))
        pro_pos = self.pro_fea(self.encoder(batch['contrast_img'].to(device), pool=True))
        ...
        mask = ids_shuffle >= (B//2)
        
        # 50% proto and 50% sample
        pro_sample = mask*pro_pos + ~mask*pro_query
        
        nce_emb_guidance = torch.stack([pro_emb, pro_sample], dim=1) #[B, 2, C]
